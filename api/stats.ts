...
async function handleStats(req: VercelRequest, res: VercelResponse) {
  ...
  if (seriesId) {
    const [total, today, sources, categories] = await Promise.all([
      db.execute({ sql: `SELECT COUNT(*) as count FROM articles a WHERE a.series_id = ?`, args: [seriesId] }),
      db.execute({ sql: `SELECT COUNT(*) as count FROM articles a WHERE a.series_id = ? AND date(a.published_at) = date('now')`, args: [seriesId] }),
    ...
    return res.status(200).json({
      ...
      categories: categories.rows.map((r) => ({ name: (r.name as string) || '未分類', count: r.count as number })),
    });
  }
...
async function handleDaily(req: VercelRequest, res: VercelResponse) {
  ...
  if (seriesId) {
    const result = await db.execute({
      sql: `SELECT date(a.published_at) as date, COUNT(*) as count FROM articles a WHERE a.published_at >= datetime('now', ?) AND a.series_id = ? GROUP BY date(a.published_at) ORDER BY date DESC`,
      args: [`-${days} days`, seriesId],
    });
    return res.status(200).json(result.rows.map((row) => ({ date: row.date as string, count: row.count as number })));
  }

  const result = await db.execute({
    sql: `SELECT date(published_at) as date, COUNT(*) as count FROM articles WHERE published_at >= datetime('now', ?) GROUP BY date(published_at) ORDER BY date DESC`,
    args: [`-${days} days`],
  });
  return res.status(200).json(result.rows.map((row) => ({ date: row.date as string, count: row.count as number })));
}
